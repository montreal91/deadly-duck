...
class DdMessage( db.Model ):
    ...
    subject_c = db.Column( db.String( 64 ) ) # @UndefinedVariable
    text_txt = db.Column( db.Text, nullable=False ) # @UndefinedVariable
    is_read = db.Column( db.Boolean, default=False ) # @UndefinedVariable

    timestamp_dt = db.Column( db.DateTime, default=datetime.utcnow() ) # @UndefinedVariable

    from_user = db.relationship( "DdUser", foreign_keys=[from_pk] ) # @UndefinedVariable
    to_user = db.relationship( "DdUser", foreign_keys=[to_pk] ) # @UndefinedVariable
...
class DdDaoMessage( object ):
    ...
    def SaveMessage( self, message=None ):
        db.session.add( message ) # @UndefinedVariable
