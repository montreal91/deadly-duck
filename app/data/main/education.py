...
faculty_registrations = db.Table(
    "faculty_registrations",
    db.Column("user_pk", db.Integer, db.ForeignKey("users.pk")),
    db.Column("faculty_pk", db.Integer, db.ForeignKey("faculties.pk"))
)

university_registrations = db.Table(
    "university_registrations",
    db.Column("user_pk", db.Integer, db.ForeignKey("users.pk")),
    db.Column("university_pk", db.Integer, db.ForeignKey("universities.pk"))
)
...
class DdFaculty(db.Model):
    ...
    name_c = db.Column(db.String(64), nullable=False, index=True)
    university_pk = db.Column(db.Integer, db.ForeignKey("universities.pk"))

    university = db.relationship("DdUniversity", backref="faculties")
    students = db.relationship(
        "DdUser",
        secondary=faculty_registrations,
        backref=db.backref("faculties", lazy="dynamic"),
        lazy="dynamic"
    )
...
class DdUniversity(db.Model):
    ...
    pk = db.Column(db.Integer, primary_key=True)
    name_c = db.Column(db.String(64), unique=True, nullable=False, index=True)
