...
class DdFriendship(db.Model):
    ...
    friend_one_pk = db.Column(db.Integer, db.ForeignKey("users.pk"))
    friend_two_pk = db.Column(db.Integer, db.ForeignKey("users.pk"))
...
class DdDaoFriendship(object):
    ...
    def GetNumberOfActiveFriendRequests(self, user_one_pk=0, user_two_pk=0):
        ...
        return query_res["number_of_active_friend_requests"]
    ...
    def GetNumberOfOutcomingFriendRequests(self, user_pk=0):
        query_res = db.engine.execute(
            text(NUMBER_OF_OUTCOMING_FRIEND_REQUESTS_SQL).params(user_pk=user_pk)).first()
        return query_res["outcoming_friend_requests"]
